    ...
    var body: some View {
        VStack(spacing: 0) {
            Group {
                ...
                case .income:
                    FinanceDashboardView(mode: .income, isOverlayPresented: $isAnyOverlayPresented)
                case .expense:
                    FinanceDashboardView(mode: .expense, isOverlayPresented: $isAnyOverlayPresented)
                case .analytics:
                    PlaceholderScreen(title: "Аналитика")
                case .goals:
                    GoalsView(isOverlayPresented: $isAnyOverlayPresented)
                }
            }
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    Text("Цели")
                        .font(.playfairDisplay(48, weight: .bold))
                        .foregroundStyle(Color(.green))
                    Spacer()
                }
                .padding(.top, -8)
        ...
        .onChange(of: replenishingGoal != nil) { _ in
            syncOverlayState()
        }
        .onDisappear {
...
private extension GoalsView {
    ...
    func replenish(goalId: UUID) {
        ...
        goals[index].history.append(
            GoalHistoryEntry(date: .now, amount: delta)
        )
        saveGoals()
        closePopups()
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            GeometryReader { proxy in
                ...
                ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 0)
                        .fill(Color(.green))
                        .frame(width: filledWidth, height: 32)
